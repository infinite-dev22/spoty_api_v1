...
public interface PurchaseReturnMasterRepository extends PagingAndSortingRepository<PurchaseReturnMaster, Long>, JpaRepository<PurchaseReturnMaster, Long> {
    @Query("SELECT prm FROM PurchaseReturnMaster prm WHERE prm.tenant.id = :tenantId " +
            "AND TRIM(LOWER(prm.ref)) LIKE %:search% AND (prm.approved = true OR prm.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND prm.latestApprovedLevel = a.level - 1) > 0)")
    ArrayList<PurchaseReturnMaster> searchAll(@Param("tenantId") Long tenantId, @Param("search") String search);

    @Query("select prm from PurchaseReturnMaster prm where prm.tenant.id = :tenantId AND (prm.approved = true OR prm.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND prm.latestApprovedLevel = a.level - 1) > 0)")

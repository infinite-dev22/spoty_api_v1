...
public interface RequisitionMasterRepository extends PagingAndSortingRepository<RequisitionMaster, Long>, JpaRepository<RequisitionMaster, Long> {
    @Query("SELECT rm FROM RequisitionMaster rm WHERE rm.tenant.id = :tenantId " +
            "AND TRIM(LOWER(rm.ref)) LIKE %:search% AND (rm.approved = true OR rm.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND a.level = rm.nextApprovedLevel) > 0)")
    ArrayList<RequisitionMaster> searchAll(@Param("tenantId") Long tenantId, @Param("search") String search);

    @Query("select rm from RequisitionMaster rm where rm.tenant.id = :tenantId AND (rm.approved = true OR rm.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND a.level = rm.nextApprovedLevel) > 0)")

...
public interface AdjustmentMasterRepository extends PagingAndSortingRepository<AdjustmentMaster, Long>, JpaRepository<AdjustmentMaster, Long> {
    @Query("SELECT am FROM AdjustmentMaster am WHERE am.tenant.id = :tenantId " +
            "AND TRIM(LOWER(am.ref)) LIKE %:search% AND (am.approved = true OR am.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND a.level = am.nextApprovedLevel) > 0)")
    List<AdjustmentMaster> searchAll(@Param("tenantId") Long tenantId, @Param("search") String search);

    @Query("SELECT am FROM AdjustmentMaster am WHERE am.tenant.id = :tenantId AND (am.approved = true OR am.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND a.level = am.nextApprovedLevel) > 0)")

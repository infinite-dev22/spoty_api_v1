...
public interface PurchaseMasterRepository
    ...
    @Query(
        ...
        "AND TRIM(LOWER(pm.ref)) LIKE %:search% AND (pm.approved = true OR pm.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND a.level = pm.nextApprovedLevel) > 0)"
    )
    ...
    @Query(
        ...
        "(SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND a.level = pm.nextApprovedLevel) > 0)"
    )

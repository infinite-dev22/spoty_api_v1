...
public interface PurchaseMasterRepository extends PagingAndSortingRepository<PurchaseMaster, Long>, JpaRepository<PurchaseMaster, Long> {
    @Query("SELECT DATE_PART('year', CAST(pm.createdAt AS date)) AS period, SUM(pm.amountPaid) AS totalValue " +
    ...
    @Query(value = "SELECT TO_CHAR(TO_DATE(CONCAT(EXTRACT(YEAR FROM CURRENT_DATE), '-', months.month, '-01'), 'YYYY-MM-DD'), 'YYYY Month') AS period, COALESCE(SUM(pm.amount_paid), 0) AS totalValue " +
    ...
    @Query("SELECT CAST(pm.createdAt AS date) AS period, SUM(pm.amountPaid) AS totalValue " +
            "FROM PurchaseMaster pm " +
            "WHERE pm.tenant.id = :id AND pm.approved = true " +
            "GROUP BY period " +

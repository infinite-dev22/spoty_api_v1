...
public interface TransferMasterRepository extends PagingAndSortingRepository<TransferMaster, Long>, JpaRepository<TransferMaster, Long> {
    @Query("SELECT tm FROM TransferMaster tm WHERE tm.tenant.id = :tenantId " +
            "AND TRIM(LOWER(tm.ref)) LIKE %:search% AND (tm.approved = true OR tm.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND tm.latestApprovedLevel = a.level - 1) > 0)")
    ArrayList<TransferMaster> searchAll(@Param("tenantId") Long tenantId, @Param("search") String search);

    @Query("select tm from TransferMaster tm where tm.tenant.id = :tenantId AND (tm.approved = true OR tm.createdBy.id = :userId OR (SELECT COUNT(a) FROM Approver a WHERE a.employee.id = :userId AND tm.latestApprovedLevel = a.level - 1) > 0)")
